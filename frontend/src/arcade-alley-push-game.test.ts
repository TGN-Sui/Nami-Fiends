...
describe('arcade-alley-push-game', () => {
  ...
  it('keeps three hazard rows, doubles waves, triples speed, and removes lane lag in skill diff', () => {
    const normal = arcadeAlleyPushGameConfig('normal');
    const hard = arcadeAlleyPushGameConfig('hard');
    const skill = arcadeAlleyPushGameConfig(ARCADE_SKILL_DIFF_MODE);

    expect(skill.hazardTracksPerLane).toBe(hard.hazardTracksPerLane);
    expect(skill.hazardTracksPerLane).toBe(3);
    expect(skill.hazardsPerLane).toBe(hard.hazardsPerLane * 2);
    expect(skill.hazardSpeedMin).toBeGreaterThan(normal.hazardSpeedMin);
    expect(skill.laneChangeCooldownMs).toBe(0);
  });
  ...
  it('labels alley push modes and detects hazard collisions', () => {
    ...
    expect(arcadeAlleyPushModeLabel(ARCADE_SKILL_DIFF_MODE)).toBe(ARCADE_SKILL_DIFF_MODE_LABEL);
...
describe('arcade-alley-push-game-store', () => {
  ...
  it('records leaderboard scores per mode', () => {
    ...
    const skillResult = recordArcadeAlleyPushGameResult({
      memberId: 'm1',
      displayName: 'Nozomi',
    ...
    expect(readArcadeAlleyPushLeaderboard(ARCADE_SKILL_DIFF_MODE)[0]?.score).toBe(20);
  });
});