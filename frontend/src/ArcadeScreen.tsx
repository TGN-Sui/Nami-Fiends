...
export function ArcadeScreen(props: ArcadeScreenProps): ReactElement {
  ...
  useEffect(() => {
    if (screenPhase === 'attract' || screenPhase === 'cabinet-select') {
      setArcadeStageCabinetId(null);
    }
  }, [screenPhase]);
  ...
  const exitToHub = useCallback(
    (page: Extract<HubDestinationPage, 'hub' | 'gamehub'>): void => {
      clearArcadeSession();
      props.onExitToHub(page);
    },
    [props],
  );
      ...
      <>
        ...
        >
          ...
          <div className="arcade-screen-cabinet-controls">
            <button
              className="arcade-screen-play-button arcade-screen-start-game-button"
  ...
  return (
    <>
      ...
      {screenPhase === 'cabinet-active' ? (
        <div
          className={
            'arcade-screen arcade-screen-cabinet-active' +
