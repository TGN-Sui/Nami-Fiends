...
type OwnerAssetSlotCatalogProps = {
  slots?: OwnerAssetSlot[];
  sections?: OwnerAssetSlotSection[];
  compact?: boolean;
};

function renderSlotCard(
  slot: OwnerAssetSlot,
  persistedAssets: Record<string, string>,
  editModeActive: boolean,
  owner: string | null,
  openSlotPicker: (slotId: string) => void,
): ReactElement {
  ...
  const arcadeMedia =
    slot.id === ARCADE_BACKGROUND_SLOT_ID
      ...
      : slot.id === ARCADE_STAGE_BACKGROUND_SLOT_ID
        ...
        : cabinetParsed
          ? resolveArcadeCabinetMedia(cabinetParsed.cabinetId, cabinetParsed.kind, storedValue)
          : null;
  const musicPreviewUrl = isArcadeMusicSlotId(slot.id) ? resolveArcadeMusicUrl(storedValue) : null;
  const previewUrl =
    ...
    (slot.id === ARCADE_BACKGROUND_SLOT_ID
      ...
      : slot.id === ARCADE_STAGE_BACKGROUND_SLOT_ID
        ...
        : isArcadeCabinetMediaSlotId(slot.id)
          ? isArcadeCabinetMediaVideoRef(slot.id, storedValue) || arcadeMedia?.kind === 'video'
          : false);
  ...
  return (
    <article className="nami-owner-asset-card" key={slot.id}>
      <button
        className={'nami-owner-asset-card-button' + (previewUrl ? ' has-owner-asset-image' : '')}
...
export function OwnerAssetSlotCatalog(props: OwnerAssetSlotCatalogProps): ReactElement {
  ...
  async function handleFileChange(event: ChangeEvent<HTMLInputElement>): Promise<void> {
    ...
    const slot = readOwnerAssetSlot(slotId);
  ...
  const catalogClassName =
    'nami-owner-asset-catalog nami-owner-advanced-scroll-region' +
    (props.compact ? ' is-compact-dense' : '');

  return (
    <>
      {props.sections
        ? props.sections.map((section) => (
            <section className="nami-owner-asset-section" key={section.id}>
              <header className="nami-owner-asset-section-head">
                <h3>{section.label}</h3>
                <p>{section.hint}</p>
              </header>
              <div className={catalogClassName} key={section.id + '-catalog-' + mediaVersion}>
                {section.slots.map((slot) =>
                  renderSlotCard(slot, persistedAssets, editMode.active, owner, openSlotPicker),
                )}
              </div>
            </section>
          ))
