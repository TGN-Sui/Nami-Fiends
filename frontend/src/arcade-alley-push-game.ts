...
import {
  ...
  ARCADE_SKILL_DIFF_ACTION_COOLDOWN_MS,
  ARCADE_SKILL_DIFF_MODE,
  ARCADE_SKILL_DIFF_MODE_LABEL,
} from './arcade-skill-diff.js';

export type ArcadeAlleyPushMode = 'normal' | 'hard' | typeof ARCADE_SKILL_DIFF_MODE;
...
export function arcadeAlleyPushGameConfig(mode: ArcadeAlleyPushMode): ArcadeAlleyPushGameConfig {
  if (mode === 'hard') {
    return {
      ...
      laneChangeCooldownMs: ARCADE_ALLEY_PUSH_LANE_CHANGE_COOLDOWN_MS,
    };
  }
  ...
  if (mode === ARCADE_SKILL_DIFF_MODE) {
    return {
      mode,
      hazardSpeedMin: arcadeSkillDiffScaledSpeed(0.12),
      hazardSpeedMax: arcadeSkillDiffScaledSpeed(0.24),
      hazardTracksPerLane: 3,
      hazardsPerLane: arcadeSkillDiffDoubledSpawnCount(1),
      playerMoveSpeed: 0.52,
      laneChangeCooldownMs: ARCADE_SKILL_DIFF_ACTION_COOLDOWN_MS,
    };
  }
...
export function applyArcadeAlleyPushLaneStep(
  ...
  if (now - lastLaneChangeAt < config.laneChangeCooldownMs) {
