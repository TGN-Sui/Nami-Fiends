#!/usr/bin/env node
/**
 * Extract packageId + AdminCap from a sui publish JSON into deployments/<network>/latest.json
 * Usage: node scripts/extract-deployment.mjs <network> <publish-json-path>
 */
...
const publishPath = process.argv[3];

if (!network || !publishPath) {
  console.error('Usage: node scripts/extract-deployment.mjs <network> <publish-json-path>');
  process.exit(1);
}
...
const outPath = path.join(rootDir, 'deployments', network, 'latest.json');
